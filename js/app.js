...
(function () {
  ...
  function renderGems() {
    ...
    html += orbs.length ? `<div class="orbs">${orbs.map((o) => { const sk = GM.orbSkills[o.n]; return `<div class="orb r${o.r}" title="${sk ? esc(sk.desc) : ''}"><i style="background:${META.elements[o.el]?.color || '#999'}"></i>${esc(o.n)}${o.el === els[0] ? ' <span class="tag gold" style="margin-left:2px">×1.5</span>' : ''}<small>${GM.rarityKo[o.r]}${o.craft ? ' · 제작소' : o.ev ? ' · ' + esc(o.ev) : GM.orbRate[o.r] ? ' · ' + GM.orbRate[o.r] : ''}${sk && sk.power ? ' · 위력 ' + sk.power : ''}</small></div>`; }).join('')}</div>
  ...
  function renderGacha() {
    ...
    const withDragon = EV.pickup.filter((e) => e.dragonIds?.length); // 보주만 있는 픽업은 드래곤 대상이 없으므로 건너뛴다
    const active = withDragon.find((e) => { const n = new Date(); return n >= parseDate(e.start) && n <= parseDate(e.end); }) || withDragon[0] || EV.pickup[0];
  ...
  function renderFeatured() {
    ...
    const pickHtml = `<div class="feat feat-pick"><div class="feat-head"><span>✨</span><b>픽업 뽑기</b>${pkShown ? whenHtml(pkShown) : ''}</div>
      ${pkShown ? `<div class="feat-dragons">${pkShown.dragonIds.map((id) => { const d = byId.get(id); return d ? dragon(id, `${els(d)} · ${obtainable(d) ? '교배로도 가능 →' : '뽑기 전용'}`, obtainable(d)) : ''; }).join('')}</div>${(() => { const now = new Date(); const live = (EV.pickup || []).filter((e) => e.orbs?.length && now >= parseDate(e.start) && now <= parseDate(e.end)); const rows = live.length ? live : pkShown.orbs?.length ? [pkShown] : []; return rows.map((e) => `<div class="feat-orbs">보주 픽업: <b>${e.orbs.map(esc).join('</b>, <b>')}</b> <small>${e.end.slice(5, 10).replace('-', '.')}까지</small></div>`).join(''); })()}${pk ? '' : '<div class="feat-orbs">최근 항목입니다. 새 픽업이 시작되면 data/events.js 를 갱신하세요.</div>'}` : '<div class="none">등록된 픽업 정보가 없습니다.</div>'}</div>`;
