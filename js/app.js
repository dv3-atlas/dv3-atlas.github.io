...
(function () {
  ...
  function openDex(d) {
    ...
    const cond = []; if (d.reqUserLv > 1) cond.push(`테이머 Lv ${d.reqUserLv}`); if (d.reqEleIds?.length) cond.push(`부모 속성에 ${d.reqEleIds.map((e) => META.elements[e]?.ko || e).join('·')} 포함`); if (d.reqEleCnt) cond.push(`부모 속성 ${d.reqEleCnt}종 이상`); if (BOOKS.includes(d)) cond.push('비밀 교배서');
